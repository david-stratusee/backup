...
#define NRREQUESTS (PIPELEN * MAXNUM)
...
int main(int argc, char *argv[])
{
    ...
    curl_multi_setopt(multi_handle, CURLMOPT_MAX_PIPELINE_LENGTH, (long)(MAXNUM));
	...
	set_share_handle(handles[i]);
        ...
        //curl_easy_setopt(handles[i], CURLOPT_TCP_NODELAY, 1L);
    ...
    for (;;) {
        ...
        for (n = still_running; n < PIPELEN * MAXNUM && added < NRREQUESTS; ++n) {
