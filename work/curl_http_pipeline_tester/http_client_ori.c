...
{
    CURLMcode ret = curl_multi_add_handle(multi_handle, handles[*added]);
    ++*running;
    print_time_prefix();
    printf("Request #%d    added [now running: %d], ret: %d\n", *added, *running, ret);
    ++*added;
}
...
int main(int argc, char *argv[])
{
    ...
    char url[256];
    snprintf(url, sizeof(url), "http://%s:%d/static/code1.py", hostname, port);
    ...
    curl_multi_setopt(multi_handle, CURLMOPT_MAX_PIPELINE_LENGTH, (long)4L);
    curl_multi_setopt(multi_handle, CURLMOPT_MAX_TOTAL_CONNECTIONS, PIPELEN);
    curl_multi_setopt(multi_handle, CURLMOPT_MAXCONNECTS, PIPELEN);
    ...
    for (int i = 0; i < NRREQUESTS; ++i) {
        ...
        curl_easy_setopt(handles[i], CURLOPT_TCP_KEEPALIVE, 1L);
        curl_easy_setopt(handles[i], CURLOPT_TCP_KEEPIDLE, 30L);
        curl_easy_setopt(handles[i], CURLOPT_TCP_KEEPINTVL, 30L);
...
printf("first batch is running\n");
    ...
    for (;;) {
        ...
        for (n = still_running; n < PIPELEN * 4 && added < NRREQUESTS; ++n) {
